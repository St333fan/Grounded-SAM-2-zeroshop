...
parser = argparse.ArgumentParser(description='Process real object dataset for segmentation')
parser.add_argument('--dataset_path', type=str, default='/code/datasets/ycbv_real_subset',
                    help='Path to the dataset directory (default: /code/datasets/ycbv_real_subset)')
parser.add_argument('--prompt', type=str, default='object in the middle.',
                    help='Text prompt for object detection (default: "object in the center of the image.")')
args = parser.parse_args()

# Set the dataset root directory from command-line argument
...
"""
# Check GPU compute capability and use appropriate dtype
gpu_compute_capability = torch.cuda.get_device_properties(0).major
print(f"GPU Compute Capability: {gpu_compute_capability}.{torch.cuda.get_device_properties(0).minor}")

# Use float16 instead of bfloat16 for better compatibility
# bfloat16 requires Ampere (8.0) or newer GPUs
torch.autocast(device_type="cuda", dtype=torch.float16).__enter__()

if gpu_compute_capability >= 8:
...
for obj_dir in obj_dirs:
    ...
    # Set up text prompt from command-line argument
    text = text_prompt
